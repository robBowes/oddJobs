/**
* @file contains the functions neccessary for the oddJobs API
* @author Robert Bowes
* @version 1.0
*/
...
/**
* deepUser -   creates a deep object that can be used by the
*              frontend to generate all the necessary pages for a user
*
* @param {Model} Job The jobs model from mongoose
* @param {Instance} user The user you need to generate a dep object for
* @param {Model} User The user model from mongoose
* @return {Object} A deep object containing a users pairted
*      jobs patron jobs and distance from each job
*/
const deepUser = async (Job, user, User) => {
    /*
    Creates a function that will search the user model for a specified user
    */
    let findUserInner = findUser(User);
    /*
    Returns an array of jobs where the target user is the patron of the job
    */
    let userIsPatron = await Job.find({patronId: user.id})
    // Cleans the instances into objects
    .lean();
    /*
    Returns an array of jobs where the target user has paired with the job
    */
    let userIsPair = await Job.find({pairedHelpers: user.id}).lean();
    // Clean the target user instance into a javascript object
    let returnUser = user.toObject();
    /*
    Job objects should only contain chat rooms that pertain to the user.
    Map through the jobs, and filter out messages that don't belong to
    the user.
    */
    ...
    /*
    Map through the array of jobs where the user is a helperand add a distance
    to the user's location and replace the array or patron id's with clean
    patron user objects
    */
    ...
    /*
    Map through jobs where user is patron and add a distance and convert
    helper ID's into objects
    */
    ...
    // Resolve app promises before returning the objects
...
/**
* Modifies a atarget user with new information
*
* @param {any} user The target user to be modidfied
* @param {any} newProps The data in the user to be modified
* @return {Object} With a status property and a payload
*/
const modify = async (user, newProps) =>{
    if (!user) return {status: false, reason: 'no user found'};
    if (!newProps) return {status: false, reason: 'no new information sent'};
    try {
...
/**
*
* @param {Model} Job The jobs model generated by the mongoose schema
* @param {Model} User The users model generated bythe mongoose schema
* @param {Object} fb The facebook response object generated by the
*  Facebook login component on the front end
* @param {String} cookie Cookie from the frontend request
* @param {any} user
* @return {Object} A reply with a status and a payload
*/
login = (Job, User)=> async (fb, cookie, user) => {
    if (!user &&
        (!fb.id ||
            !fb.name ||
            !fb.accessToken
...
const backOut = (Job, User) => async (user, body) => {
    if (!user) return {status: false, reason: 'no user information'};
    if (!body) return {status: false, reason: 'no job information'};
    if (!Job || !User) return {status: false, reason: 'server error'};
